...
public class DlxBlogIndexEventConsumer {

    private final DeadMessagePersistenceService deadMessagePersistenceService;

    @RabbitListener(queues = RabbitMQConstant.BLOG_INDEX_DLX_QUEUE)
    public void dlxInteractionEventConsumer(final Message message) {
        ...
        try {
            ...
            log.error("保存死信消息失败,消息ID{}，消息者队列{}",eventId,message.getMessageProperties().getConsumerQueue());
